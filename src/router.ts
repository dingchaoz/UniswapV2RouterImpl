...
function printAllPathsUtil(u, d, isVisited, localPathList: any[], maxHops, allPaths: any[]) {
  if (u == d) {
    // if match found then no need to
    // traverse more till depth
    console.log(`find path`, localPathList)
    allPaths = allPaths.concat(localPathList)
    results.push(allPaths)
  ...
  for (let i = 0; i < adjList[u].length; i++) {
    if (!isVisited[adjList[u][i]] && maxHops >= 0) {
      ...
      printAllPathsUtil(adjList[u][i], d, isVisited, localPathList, maxHops - 1, allPaths)
